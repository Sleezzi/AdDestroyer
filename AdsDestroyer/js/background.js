...
chrome.webRequest.onBeforeRequest.addListener(details => {
	...
	for (url in blockedURL.blockedURL) {
		...
		(!blockedURL.blockedURL[url].ressourceTypes || blockedURL.blockedURL[url].ressourceTypes.includes(details.type))) {
			if (blockedURL.blocked[details.initiator] !== "undefined") {
				...
				blockedURL.blocked[details.initiator].at[(new Date().getDate())--]++;
			}

			blockedURL.blocked.total++;
			console.log(`Request "${`${details.url}`.replace("https://", "").replace("www.", "").replace("http://", "")}" blocked from "${details.initiator}" (method: '${details.method}')`, );
