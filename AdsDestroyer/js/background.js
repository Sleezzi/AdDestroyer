...
chrome.webRequest.onBeforeRequest.addListener(details => {
	...
	for (url in blockedURL.blockedURL) {
		...
		(!blockedURL.blockedURL[url].ressourceTypes || blockedURL.blockedURL[url].ressourceTypes.includes(details.type))) {
			if (blockedURL.blocked[details.initiator] !== undefined || blockedURL.blocked[details.initiator] !== null || blockedURL.blocked[details.initiator] !== "undefined" || blockedURL.blocked[details.initiator] !== "null") {
				if (blockedURL.blocked[details.initiator]) {
					blockedURL.blocked[details.initiator].total++;
					blockedURL.blocked.total++;
				} else blockedURL.blocked[details.initiator] = { total: 1, at: [ 0, 0, 0, 0, 0, 0 ]};
				blockedURL.blocked[details.initiator].at[(new Date().getDay())-1]++;
			}
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {	
	...
	if (`${message}`.startsWith("getBlockedURL")) {
		if (`${message}`.startsWith("getBlockedURL$website=")) {
			sendResponse(blockedURL.blocked[`${message}`.replace("getBlockedURL$website=", "")]);
		} else sendResponse(blockedURL.blocked);
	}
